...
void Start()
    ...
    {
        other.GetComponent<ProximityActivate>().activeGrab = true;
    }

    private void OnTriggerExit(Collider other)
    {
        other.GetComponent<ProximityActivate>().activeGrab = false;
    }

   /* void IsTargetGrabbed(OVRGrabbable other)
    {
        if
        (other.isGrabbed)
            activeGrab = true;
    ...
    void Update()
    {
        ...
        target.alpha = Mathf.Clamp01(target.alpha + alpha * Time.deltaTime * 10);
        if (infoPanel != null)
        {
